        ...
        launch {
            try {
                val posts = getPosts(client)
                    .map { post ->
                        async {
                            PostWithAuthor(post, getAuthors(client, post.authorId))
                        }
                        async {
                            val comments = PostWithComment(post, getComments(client, post.id))
                                .comment.map { CommentsWithAuthor(it, getAuthors(client, it.authorId)) }
                        }
                    }.awaitAll()
                println(posts)
            } catch (e: Exception) {
